...
def load_wavs(wav_list, sr):
    ...
    wavs = [None]*len(wav_list)
    for list_idx, file_path in enumerate(tqdm(wav_list,desc='Wav Reading')):
...
def lps_rebuild(lpss, info):
    ...
    if isinstance(lpss, list):
        ...
        phase, mean, std, frame_size, overlap= info['phase'], info['mean'], info['std'], info['frame_size'], info['overlap'] 
        lpss = (lpss*std)+mean
